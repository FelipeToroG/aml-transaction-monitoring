...
The model is a hybrid of an unsupervised Isolation Forest anomaly head and a supervised gradient-boosted classifier, combined by stacking: the Isolation Forest score is appended as a feature column the supervised classifier consumes alongside the engineered features, and the classifier's calibrated probability over that augmented matrix is the risk score.
...
| **Random Forest** | Stable baseline; less overfitting risk | Lower accuracy on this benchmark; ran in v0 but dropped from the v1 sweep as cost-prohibitive (~12h CPU per sweep) |
| **Logistic Regression** | Linear baseline | ~10% behind boosted methods; dropped from the v1 sweep after hanging on a degenerate SAGA configuration (see INCIDENT_REPORT.md) |
...
The v1 Optuna sweep evaluates two families, XGBoost and LightGBM (Random Forest and Logistic Regression were dropped after v0; Naive Bayes and MLP are documented known underperformers on this class of problem). The training driver selects whichever family achieves the highest validation-set cost-weighted Precision@k, the cost-aware metric defined in §5.

**Why LightGBM won the v1 sweep** over XGBoost? The two finished within roughly $180 per investigator-hour of each other on the cost-weighted Precision@k objective (LightGBM 77,681 vs XGBoost 77,860, negated USD per hour), with LightGBM ahead, and LightGBM trains faster, which matters for sweep iteration. The margin is small, so the choice is empirical rather than categorical: XGBoost has more mature production tooling and CatBoost handles categoricals natively, and the factory pattern in `src.models.classifier` makes swapping the family trivial (only the family name and hyperparameter dict change) if a different cohort or cost calibration favors another.

### 4.4 Calibration: isotonic regression

Tree ensembles produce systematically over-confident predictions on imbalanced data. A "0.9" probability from XGBoost on a 0.1%-positive dataset is empirically closer to 60-70% actual positive rate. For cost-sensitive AML scoring, miscalibration distorts the cost arithmetic (which depends on the scores being meaningful probabilities) and misleads investigators (who learn to discount the displayed probability).
...
### 4.5 Ensemble combination: stacking

v1 combines the two heads by stacking. The `AnomalyScoreAppender` transformer fits the Isolation Forest inside the feature pipeline and appends its calibrated anomaly score as one additional feature column:

```
X_augmented = hstack([engineered_features, anomaly_score])
risk_score  = calibrated_classifier.predict_proba(X_augmented)[:, 1]
```

The supervised model then learns the weighting of the anomaly signal empirically, rather than having it imposed by a hardcoded constant.

**Why stacking and not the v0 weighted blend?** v0 used a fixed convex sum, `0.35 * anomaly + 0.65 * supervised`. It underperformed for a concrete reason: the Isolation Forest score range turned out narrow in practice (roughly 0.32 to 0.42), so at a fixed weight the anomaly term behaved like a near-constant offset that diluted the supervised signal instead of complementing it. Appending the score as a feature lets the gradient-boosted model use it non-linearly and only where it helps. On held-out data the stacked anomaly score ranks among the model's most important features, and the change moved Precision@k from 0.55 in v0 to 0.91 in v1.

**Why not rank fusion or a learned meta-model?** Rank-based fusion (Borda count, reciprocal rank fusion) addresses incomparable score scales, which stacking sidesteps by letting the classifier consume the raw score. A separate meta-model would add another model to tune and monitor; stacking achieves the same learn-the-combination benefit with one extra feature column and no new artifact.
...
After selecting the winning family, the training driver sets the decision threshold at the fixed operational alert capacity. The threshold is the k-th largest validation score (k = 384), the score that admits exactly k alerts and no more. At a fixed alert budget there is nothing to sweep: the threshold is a capacity constraint, computed as a rank statistic over the validation scores.

**Why fixed k and not a cost sweep over candidate thresholds?** The model deploys at exactly one operating point, the team's daily review capacity. Tuning at a variable k (the v0 approach, which evaluated each candidate threshold at a k equal to the count of predictions above it) rewards a near-floor flag-everything threshold and makes the validation and test objectives non-comparable. Selecting the score at the fixed k = 384 ties validation tuning and test evaluation to an identical alert budget. The v1 tuner ships with a unit test asserting it yields exactly k = 384 predictions on a synthetic input.
...
**Q: Why LightGBM and not XGBoost or CatBoost?**
A: XGBoost and LightGBM were both in the v1 Optuna sweep; the winner is selected on cost-weighted Precision@k. LightGBM won by a small margin (about $180 per investigator-hour on the cost objective) and trains faster. Because the margin is small it is an empirical pick rather than a categorical one: XGBoost has more mature production tooling and CatBoost handles categoricals natively, and the factory pattern makes swapping the family trivial if a different cohort favors another. (Random Forest and Logistic Regression ran in v0 but were dropped for v1; see §4.3.)

**Q: Why Isolation Forest specifically?**
A: It is the standard production anomaly detector at this data scale: linear time, parallelisable, deterministic with a fixed seed, no neighbourhood-distance computation. LOF is O(n²) and does not scale. One-Class SVM is quadratic. Autoencoders need GPU training and are less interpretable. The Isolation Forest's calibrated `[0, 1]` output is appended as a feature the supervised classifier consumes (stacking).
...
A: It is the validation score at the fixed operational capacity, k = 384, i.e. the k-th largest validation score, which admits exactly k alerts. There is no sweep: at a fixed alert budget the threshold is a rank statistic, not an optimisation target. Tuning at a variable k (the v0 approach) rewarded a near-floor flag-everything cut and made validation and test non-comparable; the fixed-k rule ties both to the same budget. The selected v1 threshold is 0.0417.
