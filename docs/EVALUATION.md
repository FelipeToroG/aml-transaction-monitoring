...
After selecting the winning family on cost-weighted Precision@k, the training driver sets the decision threshold at the fixed operational alert capacity. The threshold is the k-th largest validation score (k = 384), the score that admits exactly k alerts and no more; at a fixed alert budget the threshold is a capacity constraint, not a quantity to sweep.

This corrects the v0 tuner, which evaluated the objective at a variable k equal to the count of predictions above each candidate threshold and could settle on a near-floor flag-everything cut that made validation and test non-comparable (see INCIDENT_REPORT.md). The v1 tuner ships with a unit test asserting the chosen threshold yields exactly k = 384 predictions on a synthetic input. The selected v1 threshold is 0.0417.

## Per-family comparison

Two supervised families are swept by Optuna (the Isolation Forest is the anomaly head, stacked as a feature, not a swept supervised family):

| Family | Why included |
|---|---|
| XGBoost | Consistent strongest performer on imbalanced tabular data; GPU-accelerated |
| LightGBM | Competitive accuracy, faster training; won the v1 sweep |

Random Forest and Logistic Regression ran in the v0 sweep but were dropped for v1 as cost-prohibitive at 5M rows (Random Forest ~12h CPU per sweep; Logistic Regression hung on a degenerate SAGA configuration), and neither beats gradient-boosted trees on imbalanced tabular data. See docs/INCIDENT_REPORT.md.
