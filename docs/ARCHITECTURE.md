...
3. **Model layer** (`src.models`) - hybrid detector; the Isolation Forest anomaly score is stacked as a feature into the gradient-boosted classifier.
...
3. **Ensemble scoring (stacking).** The feature pipeline appends the calibrated Isolation Forest anomaly score as a trailing feature column (`AnomalyScoreAppender`); `supervised_classifier.predict_proba()` then scores the augmented matrix. The risk score is that calibrated supervised probability, and the anomaly component is read back from the trailing column. No post-hoc weighted blend.
